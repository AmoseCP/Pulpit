...
F11 是 2026-08-21 修订加入的（DEVELOPMENT_PLAN §11 第 18 条），**只作清屏降级**：
F12 注册失败（装了 JIT 调试器的机器由系统内核保留 F12）时自动改注册 F11，
状态栏与清屏按钮会明示真实键位。不要把 F11 用作新功能的键位。
...
- ✅ v1.3.0（2026-08-21）：**F10 语义修订**——从「投一次英文」改为**中/英显示语言的
  全局开关**（操作员实测判定旧语义反直觉：投完英文下个动作变回中文）。英文模式下
  翻页、F9 投放、历史复投全部保持英文，直到再按 F10；模式为会话级，启动恒中文；
  对照开启时 F10 提示无效。旧约定「F9 永远投中文」作废，`_activeTransId` 移除，
  合成统一走 `ComposePrimary`（对照 > 语言模式）。按钮文字随模式变（显示将切到的
  语言），状态栏标「语言：英文」。快速上手卡已同步
- ✅ v1.4.0（2026-08-21）：①**L7 修订**（§11 第 18 条）——白名单扩为 F7–F12 六键，
  F11 仅作清屏降级：F12 注册失败（JIT 调试器机器由系统保留 F12）时自动改注册 F11，
  清屏按钮/状态栏/日志三处明示真实键位（本机 F12 被占实测通过）；②字号上限滑块
  200 → 400（全屏后排可读性）；③修「上限往右拖字先大后小」——页脚预留原挂在
  MaxFontSize 上（上限↑→预留↑→正文↓），改为页脚行高随命中字号在二分预算里
  一起求解（`BandView.FitBody`/`FooterHeight`）。带状模式字号受带高瓶颈限制、
  上限拉高不再变大是**预期行为**（杠杆是带高滑块或全屏档），已向操作员说明。
  测试 **306 条全绿**
