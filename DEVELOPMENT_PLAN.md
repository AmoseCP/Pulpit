...
| L3 | `OverlayWindow` **默认**覆盖屏幕下三分之一带状区域，不全屏。**2026-08-20 修订**（操作员批准，见 §11 第 14 条）：新增可选第四档 `verticalAnchor=fullscreen`，覆盖整块副屏 | WPF `AllowsTransparency=true` 会关闭该窗口的硬件加速；全屏 1080p 软件渲染淡入较重。带状区域约 1920×324，软件渲染无压力；全屏档的逃生口是 `fadeMs=0` 直切（真机实测见 §11 第 14 条） |
...
| L13 | ~~v1 仅中文；英文译本 v1.1 补~~ **2026-08-21 操作员解锁**：F10 英文投放已实现，默认英文译本 NIV 2011（`text.englishCode`，未导入前回退已安装的英文译本） | F10 键位自 v1 就位（P0-9）。L9 单语上屏**不变**——F10 是原地换语言，不是双语同屏。修订留痕见 §11 |
...
| ID | 功能 | 状态（2026-08-20） |
|---|---|---|
| P1-1 | 英文译本，F10 原地换语言（副屏正显示经文时换语言保页位；副屏为空时投输入框引用；F9 换回中文） | ✅ 代码与数据都已交付（2026-08-21，操作员解锁）。NIV1984（trans_id=2）与 NIV2011（trans_id=3，MyBible 模块源，经元数据+判别经文验明）都已入库；默认即 NIV2011（`text.englishCode`）。源文件与转换脚本已 gitignore，清洗规则见 SCHEMA.md |
| P1-2 | 历史记录：本次聚会已投过的引用，可点击复投 | ✅ 代码已交付 |
| P1-3 | 外观设置面板：字号上限、底色不透明度、带状高度与垂直位置、字体 | ✅ 代码已交付 |
...
**P1-3 的两个取舍**：做成控制窗口里默认折叠的区块而不是独立窗口（少一套窗口生命周期，
且拖滑块时能直接看着预览区变）；**改动实时应用但不落盘**，落盘要点「保存为默认」——
滑块拖动时 ValueChanged 每帧触发，每次写 config.json 是没必要的磁盘噪音，而且彩排时
来回试参数不该每一步都改掉默认值。面板里额外放了 `animation.fadeMs`（0 = 直切），
让 M0 第 7 项的逃生口不必编辑 JSON 就能摸到。
...
| ID | 功能 | 状态（2026-08-20） |
|---|---|---|
| P2-1 | 关键词反查（「神爱世人」→ 出处），~~需补 FTS5~~ | ✅ 代码已交付。**「需补 FTS5」这条是错的，已删**，理由见下 |
| P2-2 | 多行歌词模式 | ✅ 代码已交付 |
| P2-3 | OBS WebSocket 联动（直播画面与现场投影分别控制） | ⛔ **需先裁定边界**：与 §9「不联网」的关系待操作员决定 |
| P2-4 | 副屏第二区域（角标、聚会主题常驻） | ✅ 代码已交付 |

**P2-4 的实现方式**：**另一个独立窗口**（`BadgeWindow`），不是在正文窗口里多开一块。
理由：「常驻」意味着正文淡入淡出、清屏都不能碰它——同一个窗口里做不到这种解耦。
复用 `OverlayWindowStyler`，所以 L5 四个扩展样式、L6 心跳、物理像素定位全都一致；
L4 同样适用（从不 Close）。两个窗口各有一个 2 秒心跳，但它们在屏幕上不重叠
（正文在底部三分之一，角标默认右上角），彼此不争 Z 序——要争的是放映软件。
角标用 `Viewbox` 是**合适**的：它是单行短文本、不需要换行，正是 Viewbox 适用的场合；
正文不能用是因为必须换行（见 §3 M2 的修订说明）。

**P2-2 的分页规则**：**空行即分页点**，不是固定几行一页。歌词天然分小节而小节长短不一，
作词人写下的空行就是「这里该翻页」，比任何固定行数都准；新增的 `lyrics.linesPerPage`（默认 4）
只用来把超长小节切开。歌词框的 `AcceptsReturn=True` **不违反 L8**——Enter 只在框内插换行，
永远不会送出，因为全窗口没有任何 `IsDefault` 按钮。投放走按钮，投放后 F7/F8/F12 照常可用。

**P2-1 为什么不用 FTS5（2026-08-20 实测修正）**

原文写「需补 FTS5」。实测两点都不成立：

1. **FTS5 的 `unicode61` 分词对中文基本不可用。** 它把每段连续 CJK 当成**一个 token**，
   而 FTS5 的短语查询只匹配整 token。在「神爱世人，甚至将他的独生子赐给他们」上实测：
   搜「神爱世人」命中（恰好是一个完整 token），搜「爱世人」「世人」「独生子」
   甚至「甚至将他」**全部 0 条**。而反查最常用的恰恰是词中间的短语。
   （`trigram` 分词器可以，但那要建索引、要写库。）
2. **补 FTS5 必须往库里写**，直接违反 L12（只读打开）与 CLAUDE.md「不要写入、不要迁移」。

**实际做法**：把 31021 行清洗版正文一次读进内存（约 2MB），去标点后做子串匹配。
实测建索引约 52ms（懒建，不搜就不花）、每次搜索约 1.5ms。这个规模上索引结构纯属浪费。

**去标点是关键**，不是锦上添花：操作员记得的是连续的一句话，不会记得逗号落在哪。
实测「神爱世人甚至」用 SQL `LIKE` 命中 **0 条**，去标点后命中 **1 条**。

**结果可复投**：每条命中带一个 `InputForm`（用 `books.short_zh` 拼成，如 `约3:16`）。
...
```json
{
  ...
  "text": { "useRawText": false, "englishCode": "NIV2011" }
}
```
...
| **全部 WPF 代码未经编译** | 首次构建可能大量报错 | 已做一次逐文件静态审查，产出 `docs/首次编译检查清单.md`（12 处外部 API 风险点，各带位置与退路）；Core 侧可在任意平台 `dotnet test` | 🔴 待首次 `dotnet build` |
...
| NIV 授权 | 法务风险 | 2026-08-21 操作员决定：默认译本改用 NIV 2011，1984 保留为回退；授权确认由操作员负责。译本 XML 源文件已 gitignore 不入仓 | 不再阻塞代码；发布前授权仍须落定 |
...
- 不做遥测；**从不自动联网、从不自动更新**。2026-08-20 修订（操作员批准，§11 第 15 条）：
  唯一允许的网络行为是操作员**手动点击**「⑨ 关于与更新」里的「检查更新」——查询本仓库的
  GitHub Releases，下载安装包并启动**可见的**安装向导；启动时与后台永不联网
- 不支持 macOS / Linux
- v1 不做主题、不做多套外观预设
...
### 2026-08-20（续）—— P1/P2 开发中发现的第 13 条

| # | 位置 | 原文 | 改为 | 依据 |
|---|---|---|---|---|
| 13 | §2 P2-1 | 关键词反查「**需补 FTS5**」 | 删掉 FTS5，改为内存去标点子串匹配 | ① FTS5 的 unicode61 把整段 CJK 当一个 token，实测「爱世人」「独生子」「甚至将他」全部 0 命中；② 补 FTS5 要写库，违反 L12 与 CLAUDE.md。实测内存方案建索引 52ms、单次搜索 1.5ms，且能匹配跨标点短语（LIKE 做不到） |

同轮还给 §2 的 P1/P2 两张表加了状态列与取舍说明（P1-2 的去重口径、P1-3 的不落盘策略、
P1-5 的分隔符选择），并把 P2-3 标为「需先裁定边界」——它与 §9「不联网」的关系不是代理能定的。

### 2026-08-20（三）—— L3 修订：垂直位置增加「居中」与「全屏」档（操作员批准）

这是 §1 锁定决策的第一次修订，按 CLAUDE.md 的规矩先报告冲突、由操作员裁定后执行。

| # | 位置 | 原文 | 改为 | 依据 |
|---|---|---|---|---|
| 14 | §1 L3 | 只覆盖下三分之一带状区域，不全屏 | **默认**带状不变；`band.verticalAnchor` 从 bottom/top 扩为 **bottom / top / center / fullscreen** 四档，fullscreen 覆盖整块副屏（忽略 heightPercent） | 操作员需求（居中用于强调、全屏用于完全遮蔽 PPT）。L3 的性能顾虑经真机实测不成立：**全屏 1920×1080 淡入实测 14 帧 / 253ms = 55.3 fps**（软件渲染、双屏 + WPS 放映中，2026-08-20，自检面板读数）——远高于 30fps 判读线，且这是最坏情况，带状档更轻。低配机器仍有双保险：自检面板直读帧率 + `fadeMs=0` 直切逃生口 |

带子尺寸相关的其余理由不变：默认档仍是 ~1920×324 带状；全屏档的黑底不透明度
仍由 `band.backgroundOpacity` 控制（拉到 1.0 才是完全遮蔽）。

### 2026-08-20（四）—— §9 修订：手动「检查更新」（操作员批准）

| # | 位置 | 原文 | 改为 | 依据 |
|---|---|---|---|---|
| 15 | §9 | 不联网，不自动更新，不做遥测 | 不做遥测、**从不自动**联网/更新；唯一允许的网络行为是操作员手动点击「检查更新」（查询本仓库 GitHub Releases → 下载 `Pulpit-Setup-*.exe` → 启动可见的安装向导） | 操作员需求：控制窗口尾部增加「⑨ 关于与更新」（版本、作者 @amoseding2025、免责声明、检查更新）。守住原禁令的本意：直播机上不得有任何**自发**的网络行为或**无人知情**的版本替换——检查只在点击时发生，安装走标准向导且弹框明示「请不要在直播进行中更新」，设置经 Inno 升级安装全程保留。`UpdateChecker` 是全项目唯一允许发起网络请求的类 |
...
### 2026-08-21 —— P1-1 解锁：英文译本与 F10 原地换语言（操作员批准）

| # | 位置 | 原文 | 改为 | 依据 |
|---|---|---|---|---|
| 16 | §1 L13 / §2 P1-1 | v1 仅中文；英文译本 v1.1 补，⏸ 被授权阻塞 | P1-1 解锁并交付：F10 英文投放（副屏正显示经文 → 原地换语言并保持页位；否则投输入框引用；F9 换回中文）。默认英文译本 **NIV 2011**（`text.englishCode=NIV2011`，操作员后续提供 XML 导入），未导入前回退任一已安装英文译本（当前 NIV 1984，code=NIV1984，trans_id=2），一个都没有时 F10 保持 P0-9 占位提示 | 操作员指令（2026-08-21）。L9 单语上屏不变；NIV 1984 XML 源文件已 gitignore（授权待落定，见 §8） |

实现要点（新会话必读）：
- **译本选取唯一落点**：`TranslationSelector.SelectEnglish`（Core，纯逻辑可单测）——
  配置 code 精确匹配（大小写不敏感，且必须 `lang='en'`）→ 回退 id 最大的英文译本 → null。
  App 启动时解析一次，写日志，传入 `ControlWindow`。
- **出处标签随译本语言**：`BibleRepository.Lookup` 按 `translations.lang` 取
  `books.name_zh` / `name_en`（英文经文出「John 3:16」）；`VerseText.BookNameZh`
  相应更名 `BookName`。反查（P2-1）仍只搜中文，`SearchableVerse` 不变。
- **原文/清洗版切换沿用屏上语言**：`ControlWindow._activeTransId` 记录当前译本，
  P1-4 的原地重投按它重查——英文经文切换正文来源不会悄悄变回中文。
- **英文空档是常态**：NIV 16 节归脚注（太 17:21 等）+ 3 处章节号出入
  （约 7 章 / 约叁 / 启 12），查不到时报错只在控制窗口、点名「英文」，副屏保持原状。
- **2011 版已导入**（2026-08-21 当日）：源是 MyBible SQLite 模块 `NIV_en.SQLite3`
  （经 info 元数据 + 五处判别经文验明确为 2011 版），命令
